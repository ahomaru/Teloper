...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    mMediaPlayer2Monitor = new MediaPlayerMonitor(self);
...
- (void)applicationWillTerminate:(NSNotification*)aNotification
{
    ...
    delete mMediaPlayer2Monitor;
    mMediaPlayer2Monitor = NULL;
...
- (IBAction)mediaPlayerGoNext:(id)sender {
    ...
    // set media player 1 source
    result = mMediaPlayers[0]->SetSource(sourceType, sourceIndex + 1);
    if (FAILED(result))
    {
        NSLog(@"Could not set media player 1 source\n");
        return;
...
- (IBAction)mediaPlayer2SourcePopupChanged:(id)sender
{
    ...
    BMDSwitcherMediaPlayerSourceType sourceType;
    int sourceIndex;
    uint32_t comboIndex = [mMediaPlayer2SourcePopup indexOfSelectedItem];
    ...
    // check we have media player 1
    if (mMediaPlayers.size() < 2)
    {
        NSLog(@"No media player 2\n");
        return;
    }
    
    // set media player 2 source
    result = mMediaPlayers[1]->SetSource(sourceType, sourceIndex);
    if (FAILED(result))
    {
        NSLog(@"Could not set media player 2 source\n");
        return;
    }
}
...
- (IBAction)mediaPlayer2GoNext:(id)sender {
    ...
    // set media player 1 source
    result = mMediaPlayers[1]->SetSource(sourceType, sourceIndex + 1);
    if (FAILED(result))
    {
        NSLog(@"Could not set media player 2 source\n");
        return;
...
- (IBAction)begin2ButtonClicked:(id)sender
{
    ...
    // toggle the 'at beginning' property, the button state will change upon notification from the switcher
    HRESULT result = mMediaPlayers[1]->SetAtBeginning();
    if (FAILED(result))
    {
        NSLog(@"Could not set 'at beginning'\n");
        return;
...
- (IBAction)previous2ButtonClicked:(id)sender
{
    ...
    // get the clip frame index
    result = mMediaPlayers[1]->GetClipFrame(&clipFrameIndex);
    if (FAILED(result))
    {
        NSLog(@"Could not get clip frame index\n");
        return;
    ...
    {
        // set clip frame index to previous frame
        result = mMediaPlayers[1]->SetClipFrame(clipFrameIndex - 1);
        if (FAILED(result))
        {
            NSLog(@"Could not set clip frame index\n");
            return;
...
- (IBAction)play2ButtonClicked:(id)sender
{
    ...
    // get the playing property
    result = mMediaPlayers[1]->GetPlaying(&playing);
    if (FAILED(result))
    {
        NSLog(@"Could not get playing\n");
        return;
    ...
    // toggle the playing property, the button state will change upon notification from the switcher
    result = mMediaPlayers[1]->SetPlaying(! playing);
    if (FAILED(result))
    {
        NSLog(@"Could not set playing\n");
        return;
...
- (IBAction)next2ButtonClicked:(id)sender
{
    ...
    {
        // set clip frame index to previous frame
        result = mMediaPlayers[1]->SetClipFrame(clipFrameIndex);
        if (FAILED(result))
        {
            NSLog(@"Could not set clip frame index\n");
            return;
...
- (IBAction)loop2ButtonClicked:(id)sender
{
    ...
    // get the loop property
    result = mMediaPlayers[1]->GetLoop(&loop);
    if (FAILED(result))
    {
        NSLog(@"Could not get loop\n");
        return;
    ...
    // toggle the loop property, the button state will change upon notification from the switcher
    result = mMediaPlayers[1]->SetLoop(! loop);
    if (FAILED(result))
    {
        NSLog(@"Could not set loop\n");
        return;
...
- (IBAction)uploadButtonClicked:(id)sender
{
	...
	if (comboIndex < mClips.size())
	{
		...
		if ([openDlg runModal] == NSOKButton)
		{
			...
			for (id filePathNS in [openDlg filenames])
			{
				std::string filePath([filePathNS cStringUsingEncoding: NSASCIIStringEncoding]);
	...
	else
	{
		...
		{
			// the user has clicked OK
			NSString* filePathNS = [openDlg filename];
			std::string filePath([filePathNS cStringUsingEncoding: NSASCIIStringEncoding]);
...
- (void)onMediaPlayerSourceChanged
{
	...
	// update the selected 2 source
	[self updateMediaPlayerPopupSelection];
    [self updateMediaPlayer2PopupSelection];
}
...
- (void)onMediaPlayer2BeginChanged
{
    ...
    // get the 'at beginning' property
    result = mMediaPlayers[1]->GetAtBeginning(&atBegining);
    if (FAILED(result))
    {
        NSLog(@"Could not get 'at beginning'\n");
        return;
...
- (void)onStillClipNameValidChanged
{
	...
	[self updateMediaPlayerPopupSelection];
    [self updateMedia2PopupItems:mMediaPlayer2SourcePopup];
    [self updateMediaPlayer2PopupSelection];
...
- (void)cleanUpConnection
{
    ...
    mMediaPlayer2Monitor->setMediaPlayer(NULL);
...
- (void)switcherConnected
{
    ...
    [mMediaPlayer2SourcePopup setEnabled:YES];
	[self enableMediaPlayerButtons:true];
    [self enableMediaPlayer2Buttons:true];
    ...
    mMediaPlayer2Monitor->setMediaPlayer(mMediaPlayers[1]);
...
- (void)switcherDisconnected
{
    ...
    [self enableMediaPlayer2Buttons:false];
    ...
    [mMediaPlayer2SourcePopup setEnabled:NO];
...
- (void)updateMediaPopupItems:(NSPopUpButton*)comboBox;
{
	...
	for (unsigned long clipIndex = 0; clipIndex < mClips.size(); ++clipIndex)
	{
		...
		// add the clip item
		NSString* itemText = [NSString stringWithFormat:@"Clip %d: %@", clipIndex + 1, clipName];
	...
	for (unsigned long stillIndex = 0; stillIndex < stillCount; ++stillIndex)
	{
		...
		// add the still item
		NSString* itemText = [NSString stringWithFormat:@"Still %d: %@", stillIndex + 1, stillName];
		[comboBox addItemWithTitle:itemText];
...
- (void)updateMediaPlayer2PopupSelection;
{
    ...
    // get the source
    result = mMediaPlayers[1]->GetSource(&sourceType, &sourceIndex);
    if (FAILED(result))
    {
        NSLog(@"Could not get media player source\n");
        return;
    ...
    if (sourceType == bmdSwitcherMediaPlayerSourceTypeClip)
    {
        ...
        [mMediaPlayer2SourcePopup selectItemAtIndex:popupIndex];
    }
    else if (sourceType == bmdSwitcherMediaPlayerSourceTypeStill)
    {
        uint32_t popupIndex = mClips.size() + sourceIndex; // still items are listed second in the combo box
    ...
    [self enableMediaPlayer2Buttons:valid];
}
...
- (void)enableMediaPlayer2Buttons:(bool)enabled
{
    ...
    [mMediaPlayer2BeginButton setEnabled:enabled];
    [mMediaPlayer2PreviousButton setEnabled:enabled];
    [mMediaPlayer2PlayButton setEnabled:enabled];
    [mMediaPlayer2NextButton setEnabled:enabled];
    [mMediaPlayer2LoopButton setEnabled:enabled];
}
